class titleMatcher extends EventEmitter {
  constructor({ workers, settingsStore }, name) {
    ...
    this.MATCHING_TIMEOUT = 10000 // 10 seconds

    this.formatTitle(name)
      .then(::this.mapTitle)
      .then(({ title, season, episode }) => {
        console.info('Searching for data on:', title, `S${season}`, `E${episode}`)
        this.timeout = setTimeout(() => this.emit('error', `Timed out attempting to match "${title}"`), this.MATCHING_TIMEOUT)
      ...
      .then(data => {
        if (settingsStore.adaptiveColorization) {
          ...
          return new Promise(resolve => this.getColors(data).then(palette => resolve({
            ...data,
            palette
          })))
        } else
      ...
      .then(data => {
        ...
        return new Promise(resolve => this.getPlayers().then(players => resolve({
          ...data,
          players
        })))
      })
  ...
  getColors({ images }) {
    ...
    const id = uuid()
    return new Promise(resolve => {
      sockets.emit('color:get', { id, image: images.poster.full })
